...
function withCustomAndroidAppName(config) {
    return withDangerousMod(config, [
        'android',
        async config => {
            // 读取 app.config.ts 中的 locales 配置
            const stdout = execSync('pnpm exec expo config --json').toString();
            const locales = JSON.parse(stdout)?.locales;
            ...
            Object.entries(locales).forEach(([locale, jsonFile]) => {
                ...
                const { CFBundleDisplayName } = JSON.parse(fs.readFileSync(localeFilePath, "utf8"));
                const appName = `${CFBundleDisplayName}${ process.env.APP_ENV === 'production' ? '' : `(${process.env.APP_ENV})`}`;
...
module.exports = withCustomAndroidAppName;
